...
let play ?level ?schedule ?timeout ?inventory ?limit ?playbook commit =
  ...
  Play_cache.get ?schedule { pool = None; timeout; level } { commit; limit; playbook; inventory }
...
let configure ?level ?schedule ?timeout commit =
  ...
  |>
  let> commit = commit in
  Configure_cache.get ?schedule { pool = None; timeout; level } { commit }
...
let run ?pool ?level commit playbook =
  ...
  |>
  let> playbook = playbook and+ commit = commit in
  let schedule =
    match Playbook.validity playbook with
    | None -> None
    | Some days -> Some (Current_cache.Schedule.v ~valid_for:(Duration.of_day days) ())
  in
